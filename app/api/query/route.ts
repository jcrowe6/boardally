...
import {
  ...
  updateAnonymousRequestCount,
} from "utils/anonymousDDBClient";
...
function getPrompt(userQuestion: string, rulebookChunks: string[]): string {
  return `
    Rulebook chunks: ${rulebookChunks
      .map((chunk) => `\n      - ${chunk}`)
      .join("")}
...
export async function POST(req: Request): Promise<Response | undefined> {
    ...
    if (now.getTime() > resetTime.getTime()) {
      ...
      if (userInfo.requestCount >= requestLimit) {
        ...
        const response = new Response(
          JSON.stringify({ answer: "Daily request limit reached" }),
          {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      const response = new Response(
        JSON.stringify({ answer: "Invalid data format" }),
        {
          headers: { "Content-Type": "application/json" },
          status: 400,
        }
    ...
    if (error instanceof ContentValidationError) {
      ...
      const response = new Response(
        JSON.stringify({ answer: "Invalid content" }),
        {
          headers: { "Content-Type": "application/json" },
          status: 422,
        }
